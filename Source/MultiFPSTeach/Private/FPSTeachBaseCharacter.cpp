...
void AFPSTeachBaseCharacter::RifleLineTrace(FVector CameraLocation, FRotator CameraRotation, bool IsMoving)
{
	...
	if(HitSuccess)
	{
		// 修改部分：将 HitResult.Actor 改为 HitResult.GetActor() 以获取被击中的 Actor
		AActor* HitActor = HitResult.GetActor();
		if (HitActor)
		{
			// 打印命中的 Actor 名称
			UKismetSystemLibrary::PrintString(GetWorld(), FString::Printf(TEXT("Hit Actor Name : %s"), *HitActor->GetName()));

			// 检查是否是 AFPSTeachBaseCharacter 类型的 Actor
			AFPSTeachBaseCharacter* FPSCharacter = Cast<AFPSTeachBaseCharacter>(HitActor);
			if (FPSCharacter)
			{
				// 修改部分：确保 DamagePlayer 函数的参数匹配
				DamagePlayer(HitResult.PhysMaterial.Get(), HitActor, CameraLocation, HitResult);  // 根据 DamagePlayer 函数的实际参数调整
			}
			else
			{
				// 打到墙壁，生成广播弹孔
...
void AFPSTeachBaseCharacter::PistolLineTrace(FVector CameraLocation, FRotator CameraRotation, bool IsMoving)
{
	...
	{
		// 修改部分：将 HitResult.Actor 改为 HitResult.GetActor() 以获取被击中的 Actor
		AActor* HitActor = HitResult.GetActor();  // 获取被击中的 Actor
		if (HitActor)
		{
			// 打印命中的 Actor 名称
			UKismetSystemLibrary::PrintString(GetWorld(), FString::Printf(TEXT("Hit Actor Name : %s"), *HitActor->GetName()));
...
//获取鼠标位置		
FVector2D AFPSTeachBaseCharacter::GetMousePosition()
{
